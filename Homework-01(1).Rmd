...
iowa.df<-separate(iowa.df,Year.Rain0.Temp1.Rain1.Temp2.Rain2.Temp3.Rain3.Temp4.Yield,
into = c('Year','Rain0','Temp1','Rain1','Temp2','Rain2','Temp3','Rain3','Temp4','Yield'),
sep = ";")
dim(iowa.df)#answer of b
colnames(iowa.df)#answer of c
iowa.df[5,7]#answer of d
iowa.df[2,]#answer of e
```
...
max(vector1)
[1] "7"  
non-erroneous result because vector1 is a char vec.
sort(vector1)
[1] "12" "32" "5"  "7" 
non-erroneous result because vector1 is a char vec.
sum(vector1)
Error in sum(vector1): 'type'(character) Wrong parameter
error because vector1 is a char vec.
...
Non numeric parameter in binary column operator
because vector2 is a char vec.
...
list4[[2]]+list4[[4]]
[1] 168
because it is a list and [[ ]] drops names and structures.
list4[2]+list4[4]
Error in list4[2] + list4[4] :
Non numeric parameter in binary column operator
because it is a list and [] does not drop names and structures.
```
...
```{r}
seq(1,10000,372)#answer of a
seq(1,10000,length.out = 50)#answer of a
rep(1:3,times=3)#answer of b
rep(1:3,each=3)#answer of b
```
    Explain:
    rep(1:3,times=3) means seq(1,3) is repeates 3 times.
    rep(1:3,each=3) means each element of seq(1,3) is repeated 3 times.
...
```{r}
deciding<-orings[c(1,2,4,11,13,18),]
ggplot(data = deciding) +
    geom_point(aes(x = Temperature, y = Total))+
    labs(x = "  temperature",
         y = " total incidents",
         title = " extracting data set")
ggplot(data = orings) +
    geom_point(aes(x = Temperature, y = Total))+
    labs(x = "  temperature",
         y = " total incidents",
...
```{r}
str(ais)# none of the columns hold missing values.
```
...
```{r}
ais%>%dplyr::mutate(num=1)%>%
    dplyr::group_by(sport,sex) %>%
    dplyr::summarize(cont_num = sum(num))%>%
    dplyr::ungroup()
```
Explain:
  there exits large imbalance in the numbers of the two sexes in Gym,Netball,T_Sprnt and W_Polo.
...
```{r}
x=matrix(c( 217,254,248,254,253,227,178,207,217,
            24387,5374,4624,2247,1353,1223,1151,755,657),ncol=2)
y<-x
x[,2]<-log2(x[,2])
dotchart(t(x),pch=16,
         xlim=c(170,280),
...
sum(area)
```
i.e. a lower bound for the area of Manitoba covered by water is 41771.
